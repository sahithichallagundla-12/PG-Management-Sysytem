...
const navItems = [
  ...
  { href: "/service?category=Electricity", label: "Electricity", icon: Zap },
  { href: "/service?category=Plumbing", label: "Plumbing", icon: Droplets },
  { href: "/service?category=Carpenter", label: "Carpenter", icon: Hammer },
  { href: "/service?category=AC%20Technician", label: "AC Technician", icon: AirVent },
  { href: "/service?category=Cleaning", label: "Cleaning", icon: Brush },
  { href: "/service?category=Internet", label: "Internet", icon: Wifi },
]
...
export default function ServiceLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated || !currentUser?.role?.toLowerCase().includes("service")) {
  ...
  const NavContent = () => (
    <>
      <div className="flex items-center gap-2.5 border-b border-border/30 px-6 py-5">
        ...
        <span className="font-semibold text-[#0B2E33]">Service Portal</span>
      </div>
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems.map((item) => {
          ...
          const isActive = itemCategory 
            ...
            : pathname === item.href && !currentCategory;
