...
const navItems = [
  ...
  { href: "/owner/complaints", label: "Complaints", icon: AlertCircle },
  { href: "/owner/payments", label: "Payments", icon: CreditCard },
  { href: "/owner/reviews", label: "Reviews", icon: Star },
  { href: "/owner/food-menu", label: "Food Menu", icon: Utensils },
]
...
export default function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated || currentUser?.role?.toLowerCase() !== "owner") {
  ...
  const NavContent = () => (
    <>
      <div className="flex items-center gap-2.5 border-b border-border/30 px-6 py-5">
        ...
        <span className="font-semibold text-[#0B2E33]">Owner Portal</span>
      </div>
